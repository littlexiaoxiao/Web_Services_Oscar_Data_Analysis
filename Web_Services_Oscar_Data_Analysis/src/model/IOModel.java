...
public class IOModel{
    ...
    public Vector<String> tokenizeComment(String comment) {
        // Step 1, remove non-word char.
        comment = comment.replaceAll("[\\W\\d_]", " ");
        comment = comment.toLowerCase();
        System.out.println("after step 1:\n" + comment);

        String[] words = comment.split("[\\s_]+");

        Vector<String> tokens = new Vector<String>();
        for (int i = 0; i < words.length; i++) {
            //        System.out.printf("before: %s\t", words[i]);
            //          System.out.printf("after: %s\n", words[i]);
            // Step 3, Stemming.
            /*
            if (words[i].length() > 0) 
                words[i] = porter.stripAffixes(words[i]);
                */
            if (words[i].length() > 0) {
